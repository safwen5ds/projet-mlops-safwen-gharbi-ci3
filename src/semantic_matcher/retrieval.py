...
class SemanticMatcher:
    def __init__(
        ...
        self.model = model
        self.config = config
        self._item_texts = [item.get("normalized_text", "") for item in items]

    @classmethod
    def from_artifacts(cls, artifacts_dir: str, model_name: Optional[str] = None, index_version: Optional[str] = None) -> "SemanticMatcher":
        ...
        return cls(index=index, items=items, bm25=bm25, model=model, config=config)

    def match(self, query: str, top_k: int = 5) -> List[Dict[str, object]]:
        normalized_query = normalize_text(query)
        tokens = normalized_query.split()
        embedding = self.model.encode([normalized_query], normalize_embeddings=True)
        ...
        semantic_norm = {idx: (score + 1.0) / 2.0 for idx, score in semantic_pairs}

        bm25_scores = self.bm25.get_scores(tokens)
        bm25_scores = np.asarray(bm25_scores, dtype="float32")
        lexical_top_n = max(top_k, self.config.lexical_top_n)
        lexical_indices = np.argsort(bm25_scores)[::-1][:lexical_top_n].tolist()
        max_lexical = float(bm25_scores[lexical_indices[0]]) if lexical_indices else 0.0
        lexical_norm = {
            idx: float(bm25_scores[idx]) / max_lexical if max_lexical else 0.0
        ...
        for idx in candidate_indices:
            ...
            confidence = calibrate_score(final_score, self.config.calibration_a, self.config.calibration_b)
            record = self.items[idx]
            overlap = _token_overlap(tokens, record.get("normalized_text", ""))
        ...
        return [result.as_dict() for result in results[:top_k]]
...
def _token_overlap(query_tokens: List[str], normalized_text: str, limit: int = 6) -> str:
